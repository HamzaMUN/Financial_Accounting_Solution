...
Public Class grn
    ...
    'Private Sub grn_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
    '    Call loadDate()
    '    Try
    ...
    '        conn2.Open()
    '        comm = New SqlCommand("SELECT * FROM voucherMaster WHERE VRStatus = @input1got AND fiscalYear = @input2got AND VRType = @input3got ORDER BY VRMasterID DESC", conn2)
    ...
    '                conn = New SqlConnection(constr)
    '                conn.Open()
    '                comm = New SqlCommand("SELECT * FROM Stock WHERE DocNo = '" & voucherNo.Text & "' AND DocType = '" & "GRN" & "' AND FiscalYear = '" & fiscalYearIDGot & "' ORDER BY id ASC", conn)
    '                reader = comm.ExecuteReader
    '                'clear list before loading records
    '                ListView1.Items.Clear()
    '                Do While reader.Read = True 'Loop to load data realtime
    ...
    '                        comm = New SqlCommand("UPDATE Stock SET Status = 'C', PostDate = '" & todaysDate & "', PostBy = '" & userNameGot & "' WHERE DocNo = " & voucherNo.Text & " AND DocType = '" & "GRN" & "' AND FiscalYear = " & fiscalYearIDGot & " ", conn)
    '                        affector = comm.ExecuteNonQuery
    ...
    '                        affector = comm.ExecuteNonQuery
    '                        comm = New SqlCommand("UPDATE Stock SET Status = 'O', PostDate = '" & todaysDate & "', PostBy = '" & userNameGot & "' WHERE DocNo = " & voucherNo.Text & " AND DocType = '" & "GRN" & "' AND FiscalYear = " & fiscalYearIDGot & " ", conn)
    '                        affector = comm.ExecuteNonQuery
    ...
    '                comm.Parameters.Add("@input1got", SqlDbType.VarChar).Value = "GRN"
    '                comm.Parameters.Add("@input2got", SqlDbType.VarChar).Value = "PJV"
    ...
    '                conn = New SqlConnection(constr)
    '                conn.Open()
    '                comm = New SqlCommand("INSERT INTO voucherMaster(VRType, VRNo, VRDate, VRPostDate, VRStatus, userName, fiscalYear, VRMonth, VRRemarks, PartyID, PartyCatID) VALUES ('" & "GRN" & "', " & voucherNo.Text & "  , '" & dateInput.Text & "'  , '" & todaysDate & "'  , '" & "O" & "'  , '" & userNameGot & "'  , " & fiscalYearIDGot & "  , '" & todaysMonth & "'  , '" & remarksInput.Text & "', '" & partyIDInput.Text & "', '" & partyCatInput.SelectedValue & "' )", conn)
    '                affector = comm.ExecuteNonQuery
    '                partyCatInput.Enabled = False
    '                dateInput.Enabled = False
    '                productIDInput.ReadOnly = False
    ...
    '                    If delresponse = MsgBoxResult.Yes Then
    '                        comm = New SqlCommand("DELETE FROM Stock WHERE DocNo = " & voucherNo.Text & " AND DocType = '" & "GRN" & "' AND FiscalYear = " & fiscalYearIDGot & " ", conn)
    '                        affector = comm.ExecuteNonQuery
    '                        comm = New SqlCommand("DELETE FROM voucherMaster WHERE VRNo = " & voucherNo.Text & " AND VRType = '" & "GRN" & "' AND VRStatus = '" & "O" & "' AND fiscalYear = " & fiscalYearIDGot & " ", conn)
    '                        affector = comm.ExecuteNonQuery
    ...
    '                    Dim delresponse = MsgBox("Are you sure you want to delete this record?", MsgBoxStyle.YesNo Or MsgBoxStyle.DefaultButton2 Or MsgBoxStyle.Critical, "Delete Record")
    '                    If delresponse = MsgBoxResult.Yes Then
    '                        comm = New SqlCommand("DELETE FROM Stock WHERE id = '" & ListView1.FocusedItem.SubItems.Item(0).Text & "' AND DocNo = " & voucherNo.Text & " AND DocType = '" & "GRN" & "' AND FiscalYear = " & fiscalYearIDGot & " ", conn)
    '                        affector = comm.ExecuteNonQuery
