...
Public Class jv
    ...
    Private Sub jv_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
        Call loadDate()
        Try
    ...
    Public Sub showData(ByVal position As Integer)
        Try
            ...
            comm.Parameters.Add("@input4got", SqlDbType.VarChar).Value = "JV"
            adapter = New SqlDataAdapter(comm)
            ...
            ElseIf tableLoad.Rows.Count() <> 0 Then
                ...
                Try
                    conn = New SqlConnection(constr)
                    conn.Open()
                    comm = New SqlCommand("SELECT * FROM voucherDetails WHERE VRNo = '" & voucherNo.Text & "' AND VRType = '" & "JV" & "' AND fiscalYear = '" & fiscalYearIDGot & "' AND VRMonth = '" & voucherMonthGot & "' ORDER BY voucherDetailID ASC", conn)
                    reader = comm.ExecuteReader
                    'clear list before loading records
                    ListView1.Items.Clear()
                    Do While reader.Read = True 'Loop to load data realtime
    ...
    Private Sub searchQ3()
        ...
        searchSupplier.parentFormGot = "jv"
    ...
    Private Sub searchQ4()
        ...
        searchEmployee.parentFormGot = "jv"
    ...
    Private Sub searchQ5()
        ...
        searchLCSupplier.parentFormGot = "jv"
    ...
    Private Sub searchQ6()
        ...
        searchCostCentre.parentFormGot = "jv"
                ...
                Else
                    If statusInput.SelectedValue = "O" And Val(differenceOutput.Text) = 0 And differenceOutput.Text <> Nothing Then
                        ...
                        If delresponse = MsgBoxResult.Yes Then
                            ...
                            comm = New SqlCommand("UPDATE voucherDetails SET VRStatus = 'C', Postby = '" & userNameGot & "' WHERE VRNo = " & voucherNo.Text & " AND VRType = '" & "JV" & "' AND fiscalYear = " & fiscalYearIDGot & " AND VRMonth = '" & voucherMonthGot & "' ", conn)
                            affector = comm.ExecuteNonQuery
                ...
                Else
                    If statusInput.SelectedValue = "C" Then
                        ...
                        If delresponse = MsgBoxResult.Yes Then
                            ...
                            affector = comm.ExecuteNonQuery
                            comm = New SqlCommand("UPDATE voucherDetails SET VRStatus = 'O', Postby = '" & userNameGot & "' WHERE VRNo = " & voucherNo.Text & " AND VRType = '" & "JV" & "' AND fiscalYear = " & fiscalYearIDGot & " AND VRMonth = '" & voucherMonthGot & "' ", conn)
                            affector = comm.ExecuteNonQuery
    ...
    Private Sub btnPrint_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnPrint.Click
        ...
        Else
            jvPrint.jvNoGot = voucherNo.Text
            jvPrint.fiscalYearGot = fiscalYearIDGot
            jvPrint.vrMonthGot = monthInput.SelectedValue
        ...
        If e.KeyCode = Keys.Enter Then
            If remarksInput.ReadOnly = False And saveType = "new" And masterBtn.Checked = True And locked = "yes" Then
                ...
                Try
                    conn = New SqlConnection(constr)
                    conn.Open()
                    comm = New SqlCommand("INSERT INTO voucherMaster(VRType, VRNo, VRDate, VRPostDate, VRStatus, userName, fiscalYear, VRMonth, VRRemarks) VALUES ('" & "JV" & "', " & voucherNo.Text & "  , '" & dateInput.Text & "'  , '" & todaysDate & "'  , '" & "O" & "'  , '" & userNameGot & "'  , " & fiscalYearIDGot & "  , '" & monthInput.SelectedValue & "'  , '" & remarksInput.Text & "' )", conn)
                    affector = comm.ExecuteNonQuery
                    dateInput.Enabled = False
                    coaCodeInput.ReadOnly = False
                    coaCodeInput.Focus()
            ...
            If tableLoad.Rows.Count() <> 0 Then
                If locked = Nothing And statusInput.SelectedValue = "O" Then
                    ...
                    If masterBtn.Checked = True Then
                        ...
                        If delresponse = MsgBoxResult.Yes Then
                            comm = New SqlCommand("DELETE FROM voucherDetails WHERE VRNo = " & voucherNo.Text & " AND VRType = '" & "JV" & "' AND fiscalYear = " & fiscalYearIDGot & " AND VRMonth = '" & voucherMonthGot & "' ", conn)
                            affector = comm.ExecuteNonQuery
                            comm = New SqlCommand("DELETE FROM voucherMaster WHERE VRNo = " & voucherNo.Text & " AND VRType = '" & "JV" & "' AND VRStatus = '" & "O" & "' AND fiscalYear = " & fiscalYearIDGot & " AND VRMonth = '" & monthInput.SelectedValue & "' ", conn)
                            affector = comm.ExecuteNonQuery
                    ...
                    ElseIf detailBtn.Checked = True And ListView1.SelectedItems.Count > 0 Then
                        Dim delresponse = MsgBox("Are you sure you want to delete this record?", MsgBoxStyle.YesNo Or MsgBoxStyle.DefaultButton2 Or MsgBoxStyle.Critical, "Delete Record")
                        If delresponse = MsgBoxResult.Yes Then
                            comm = New SqlCommand("DELETE FROM voucherDetails WHERE voucherDetailID = '" & ListView1.FocusedItem.SubItems.Item(4).Text & "' AND VRNo = " & voucherNo.Text & " AND VRType = '" & "JV" & "' AND fiscalYear = " & fiscalYearIDGot & " AND VRMonth = '" & voucherMonthGot & "' ", conn)
                            affector = comm.ExecuteNonQuery
                            MessageBox.Show("Voucher Deleted Successfully.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            showData(index)
                        End If
    ...
    Private Sub btnSave_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSave.Click
        If locked = "yes" And saveType = "update" Then
            If masterBtn.Checked = True Then
                ...
                affector = comm.ExecuteNonQuery
                comm = New SqlCommand("UPDATE voucherDetails SET VRDate = '" & dateInput.Text & "', VREditDate = '" & todaysDate & "' WHERE VRType = '" & "JV" & "' AND VRNo = '" & voucherNo.Text & "' AND fiscalYear = '" & fiscalYearIDGot & "' AND VRMonth = '" & monthInput.SelectedValue & "' ", conn)
                affector = comm.ExecuteNonQuery
                conn.Close()
